...
# =============================================================================
#   <mount_root>/
#     ExternalDrive/   DuckDB/warehouse.duckdb    100 bytes  <- decoy stub
#     ExternalDrive 1/ DuckDB/warehouse.duckdb  5,000 bytes  <- the real thing
#
...
test_that("the real warehouse is chosen over the decoy stub", {
  root <- fixture(list("ExternalDrive" = 100, "ExternalDrive 1" = 5000))
  real  <- file.path(root, "ExternalDrive 1", "DuckDB", "warehouse.duckdb")
  decoy <- file.path(root, "ExternalDrive",   "DuckDB", "warehouse.duckdb")
  before <- file.info(c(real, decoy))[, c("size", "mtime")]

  got <- resolve_file_on_volume("DuckDB/warehouse.duckdb", "ExternalDrive*",
...
test_that("the volume name does not matter", {
  for (nm in c("ExternalDrive", "ExternalDrive 1", "ExternalDrive 2")) {
...
test_that("zero candidates is an error, never a hardcoded fallback", {
  root <- withr_tempdir()
  expect_error(resolve_file_on_volume("DuckDB/warehouse.duckdb", "ExternalDrive*",
                                      min_bytes = MINB, mount_root = root),
               "no mounted volume matches")
})

test_that("only-implausible candidates is an error, not 'use the biggest'", {
  root <- fixture(list("ExternalDrive" = 100, "ExternalDrive 1" = 200))
  expect_error(resolve_file_on_volume("DuckDB/warehouse.duckdb", "ExternalDrive*",
                                      min_bytes = MINB, mount_root = root),
               "below the")
})
...
test_that("two plausible warehouses is an error, not a silent choice", {
  root <- fixture(list("ExternalDrive 2" = 5000, "ExternalDrive 3" = 6000))
  expect_error(resolve_file_on_volume("DuckDB/warehouse.duckdb", "ExternalDrive*",
                                      min_bytes = MINB, mount_root = root),
...
test_that("a bad environment override fails instead of creating anything", {
  ...
  root <- fixture(list("ExternalDrive 1" = 5000))
  ...
  withr_env("RESEARCHPATHS_ROOT", "/Volumes/MuflySamsung", {
    expect_error(resolve_volume("ExternalDrive*", mount_root = root),
                 "does not exist")
  })
})
...
test_that("the shipped DuckDB floor would reject a real stub", {
  # A few KB is the size of the empty database this failure leaves behind;
  # 12,288 bytes is a realistic instance of it.
...
test_that("resolve_volume refuses to guess between two mounted drives", {
  root <- fixture(list("ExternalDrive" = 5000, "ExternalDrive 1" = 5000))
  expect_error(resolve_volume("ExternalDrive*", mount_root = root),
               "volumes match")
  # ...but allows several when the caller will disambiguate by content, which
  # is exactly how the decoy case is resolved.
  expect_length(resolve_volume("ExternalDrive*", mount_root = root,
